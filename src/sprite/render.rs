...
//!
//! Both entry points return the target's own error unchanged: a sprite paint either
//! writes pixels or fails exactly the way the panel bus failed, and this crate names no
//! concrete panel.
...
/// without a flash. A transparent draw would instead smear, because frame N never erases
/// the pixels frame N−1 lit and N does not.
///
...
/// (`CASET`/`RASET`/`RAMWR`) 400 times a frame. Measured on real hardware, that cost
/// **85 ms** per paint against a 50 ms animation tick: the creature silently dropped
/// frames, and nothing on the glass said so. A framebuffer cannot reveal this; only the
/// hardware can.
pub fn draw_onto<D>(
    target: &mut D,
    sprite: &Sprite,
    ...
    D: DrawTarget<Color = Rgb565>,
{
    ...
    target.fill_contiguous(&area, colours)
}
...
mod tests {
    ...
    /// The test canvas. Any size the sprite fits in would do; these tests are about the
    /// renderer, not about a particular panel.
    const CANVAS: Size = Size::new(135, 240);
    ...
    fn a_sprite_drawn_off_the_canvas_escapes() {
        let fb: Framebuffer = painted(Point::new(CANVAS.width as i32, 0), 1);
        assert!(fb.escaped() > 0);
    ...
    fn the_opaque_and_compositing_paths_paint_the_same_picture() {
        ...
        let mut composited: Framebuffer = Framebuffer::new(CANVAS); // already black
        draw(&mut composited, sprite, frame, origin, 3).expect("composite over black");

        let mut opaque: Framebuffer = Framebuffer::new(CANVAS);
